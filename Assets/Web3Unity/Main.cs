...
public class Main : MonoBehaviour
{
    ...
    public GameObject emptyInventoryTxt;
    ...
    private void Start()
    {
        ...
        publicAddress.gameObject.SetActive(false);
    ...
    async private void OnConnected()
    {
        ...
        publicAddress.gameObject.SetActive(true);
        publicAddress.text = account.Substring(0, 5) + "..." + account.Substring(account.Length - 4, 4);
    ...
    async void GetInventory()
    {
        ...
        if (balance == 0)
        {
            emptyInventoryTxt.SetActive(true);
        }
            ...
            for (int j = 0; j < cardsToInstantiate; j++)
            {
                ...
                if (tempmetadatafornft["name"])
                {
                    ...
                    card.transform.GetChild(0).GetComponent<Image>().sprite =
                        nftImage[int.Parse(nftIndex.Substring(nftIndex.Length - 1)) - 1];
                    card.transform.GetChild(0).GetComponent<NFTDetails>().name = tempmetadatafornft["name"].Value;
                    card.transform.GetChild(0).GetComponent<NFTDetails>().description =
                        tempmetadatafornft["description"].Value;
                    card.transform.GetChild(0).GetComponent<NFTDetails>().maxSpeed =
                        int.Parse(tempmetadatafornft["attributes"][0]["value"].Value);
                    card.transform.GetChild(0).GetComponent<NFTDetails>().acceleration =
                        int.Parse(tempmetadatafornft["attributes"][1]["value"].Value);
                    card.transform.GetChild(0).GetComponent<NFTDetails>().nitro =
                        int.Parse(tempmetadatafornft["attributes"][2]["value"].Value);
                    card.transform.GetChild(0).GetComponent<NFTDetails>().steeringPower =
                        int.Parse(tempmetadatafornft["attributes"][3]["value"].Value);
                    card.transform.GetChild(0).GetComponent<NFTDetails>().color =
                        tempmetadatafornft["attributes"][4]["value"].Value;
                }
    ...
    {
        emptyInventoryTxt.SetActive(false);
        ClearInventory();
        refreshButton.interactable = false;
        GetInventory();
    }
